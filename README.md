...
| تحكم شامل للمالك | لوحة واحدة على `/dashboard`: ضمّ المكاتب، إيقافها، تعديل حمايتها، والاشتراكات — بسجل لكل إجراء |
...
- **لوحة الشبكة: `http://localhost:3000/`** — الجذر يحوّل إليها (لمالك المنصة فقط)
- توثيق الـ API: `http://localhost:3000/api/docs`
- فحص الصحة: `http://localhost:3000/health`

## أوامر صاحب المكتب على الواتساب

كل ما يحتاجه — لا شاشات ولا تطبيق:

| ما يرسله | ما يحدث |
|---|---|
| صور + فيديو + «فيلا للبيع النرجس ١.٨٥ مليون» + 📍 | عرض واحد مُجهّز، ثم أزرار: نشر / تعديل / إلغاء |
| `عروضي` | قائمة قابلة للضغط بآخر العروض — يختار عرضاً فتظهر أزرار الإرسال |
| `أرسل العرض FLB-002 لعملاء آخر شهر` | إرسال موجّه بأمر نصي حر |
| `أرسل آخر عرض لكل العملاء` | نفس الشيء بلا رقم عرض |
| `إلغاء الإرسال` | إيقاف إرسال جارٍ قبل أن يكتمل |
| `عملائي` · `اشتراكي` · `مساعدة` | إحصاءات وحالة الاشتراك والأوامر |

المحلّل **متحفّظ عمداً**: فعل الإرسال يجب أن يكون في **بداية** الرسالة ومعه ذكر للعملاء.
فنصّ عرض مثل «أرض للبيع القيروان ٩٥٠ الف، أرسل لي العميل اللي يسأل» يُعامل عرضاً
عقارياً لا أمراً — وإلا لضاع ما كتبه المكتب. له اختبارات تغطي هذه الحالات تحديداً.
